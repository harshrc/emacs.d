...
(defun my-lispy-up (&optional arg)
  "move up"
  (interactive)
  (if (or (lispy-left-p)
          (lispy-right-p)
...
(defun my-lispy-down (&optional arg)
  "move down"
  (interactive)
  (if (or (lispy-left-p)
          (lispy-right-p)
...
(defvar lispy-mnemonic-mode-map-base
  (let ((map (copy-keymap lispy-mnemonic-mode-map)))
    ...
    (define-key map (kbd "<left>")  #'backward-word)
    (define-key map (kbd "<right>") #'forward-word)
    (define-key map (kbd "<up>")    #'my-lispy-up)
    (define-key map (kbd "<down>")  #'my-lispy-down)
    map))
...
(defvar lispy-mnemonic-mode-map-special
  (let ((map (copy-keymap lispy-mnemonic-mode-map-base)))
    ...
    (lispy-define-key map "b" #'lispy-back) ;; lispy-mnemonic-b
